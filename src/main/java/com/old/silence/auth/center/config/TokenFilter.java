...
import com.old.silence.auth.center.security.SilenceHallGrantedAuthority;
import com.old.silence.auth.center.security.SilenceHallTokenAuthority;
...
public class TokenFilter extends OncePerRequestFilter {
    ...
    private final SilenceHallTokenAuthority tokenAuthority;

    public TokenFilter(SilenceHallTokenAuthority tokenAuthority) {
        ...
        if(StringUtils.hasText(token) && tokenAuthority.verifyToken(token)){
            ...
            if(jacksonMapper.validateJson(subject)){
                ...
                var authorities = principal.getRoles()
                        .stream()
                        .map(roleDto -> new SilenceHallGrantedAuthority(roleDto.getCode(), roleDto.getName()))
